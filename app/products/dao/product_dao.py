...
class ProductDao:
    def __init__(self):
        self.products_collection = Database().products_collection()
    ...
    def create_product(self, product_instance: ProductModel) -> InsertOneResult:
        ...
        """
        product_data_dict = product_instance.dict(by_alias=True)
    ...
    def update_product(self, product_id: str, product_instance: ProductModel) -> UpdateResult:
        ...
        """
        product_instance_to_update = self.get_product_by_id(product_id)
        if not product_instance_to_update:
            return None
        
        product_instance_id_dict = {"_id": ObjectId(product_id)}
        product_new_data_dict = { 
            "$set": {
                "nameProduct": product_instance.name_product,
                "quantityProduct": product_instance.quantity_product
            }
        }
